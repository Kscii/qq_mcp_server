...
拟定跑团回复前通常只调用一次 trpg.get_roleplay_context；同一轮已有上下文时不要重复。如果工具以 QQ_CONTEXT_STALE 拒绝，不能使用旧消息继续扮演，应先调用 trpg.get_status 并提示用户检查管理 App 的 NapCat 状态。仅当精确机制影响回答、检定建议或候选时调用 trpg.search_coc_rules。需要他人或检定决定的行为停在尝试、询问或请求。用户要求在浏览器检查已保存模组资料时调用 trpg.open_campaign_dashboard；页面只读，不替代 MCP 写入。
