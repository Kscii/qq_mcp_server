...
用 admin.list_groups 查看所有已授权群及下一步；初始化单群时用 admin.get_group_setup 返回完整但简短的检查清单。成员绑定必须先用 admin.list_group_members 获取稳定 QQ 号，数据库绑定 QQ 号而不是昵称。同一玩家可绑定多个已登记 QQ 号；每群可有多个 KP 和骰娘。群 App 停用只锁定跑团工具，不停止事件入库。新模组必须使用新 QQ 群。

消息缺口只在事件断线、心跳异常、collector 重启或用户明确报告后处理。重新在线后系统只自动补一页；完整修复先用 admin.list_message_gaps，用户明确给出时间段时可用 admin.create_message_gap，只有用户明确同意才用 admin.control_message_gap_repair。修复最多每 60 秒一页、24 小时 30 页，出错或重启后保持暂停。用户决定放弃修复时才用 admin.accept_message_gap。不要寻找关键词命令或额外 Skill，自然语言意图由工具描述路由。"""
