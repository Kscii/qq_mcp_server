...
- 正常运行只消费 NapCat HTTP-SSE，不周期查询登录、群列表、成员或历史；所有群消息入库。
- 群访问网页只决定 AI/MCP 是否可以读取该群，不影响 SSE 采集。
- SSE 会话、心跳与 OneBot 动作全部审计；断线、异常心跳和非正常重启会登记消息缺口。
- 历史只用于用户明确批准的缺口修复：全局每 60 秒最多一页、滚动 24 小时最多 30 页。
- 登录失效、账号不匹配、切号或人工暂停会持久化熔断，必须由用户完成登录后校验恢复。
...
- 普通应用发布通常与 NapCat 生命周期隔离；v0.6 首次升级会迁移当前账号登录目录。
