...
3. 发布新应用。首次 v2/v3/v4→v5 升级会先停止旧应用并创建经过校验的 SQLite 备份；
   v4→v5 不暂停 QQ 会话，数据库迁移本身不会连接 OneBot。
4. v0.6 首次账号目录迁移会停止 NapCat 一次；若账号仍冻结，不要在部署后恢复采集。
5. 新应用健康检查失败时，发布脚本先原子恢复迁移前备份，再回滚旧镜像。
6. 账号解冻并在固定设备完成登录后，先看缓存状态，等待五分钟稳定观察；缺口随后按
   每分钟一页、每天最多 30 页渐进补偿。
...
v0.8 首次发布会把本项目 schema v2/v3/v4 原子升级为 v5，并保留迁移前备份；不需要
重新授权群或上传人物卡。v4 里已启用跑团的群保持未归档，其余白名单群自动归档。
